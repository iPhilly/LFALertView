...
#define LFATitle_H      20.0f
#define LFABtn_H        30.0f
...
#define LFADTitleFont       [UIFont boldSystemFontOfSize:17];
#define LFADMessageFont     [UIFont systemFontOfSize:14];
#define LFADBtnTitleFont    [UIFont systemFontOfSize:15];



#import "LFAlertView.h"
#import "UILabel+LFAdd.h"
...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle clickIndexBlock:(LFAlertClickIndexBlock)block{
    if(self=[super init]){
        ...
        if (title) {
            _titleLab=[[UILabel alloc] initWithFrame:CGRectMake(0, 10, AlertView_W, LFATitle_H)];
            _titleLab.text=title;
            _titleLab.textAlignment=NSTextAlignmentCenter;
            _titleLab.textColor=[UIColor blackColor];
            _titleLab.font=LFADTitleFont;
        ...
        _messageLab.font=LFADMessageFont;
        ...
        CGSize labSize = [_messageLab getLabelRectWithMaxWidth:AlertView_W - messageLabSpace * 2];
        ...
        //计算_alertView的高度
        _alertView.frame=CGRectMake(0, 0, AlertView_W, _messageLab.frame.size.height+LFATitle_H+LFABtn_H+40);
        ...
        if (cancelTitle) {
            ...
            _cancelBtn.titleLabel.font=LFADBtnTitleFont;
        ...
        if (otherBtnTitle) {
            ...
            _otherBtn.titleLabel.font=LFADBtnTitleFont;
        ...
        if (cancelTitle && !otherBtnTitle) {
            _cancelBtn.tag=0;
            _cancelBtn.frame=CGRectMake(btnLeftSpace, btn_y, AlertView_W-btnLeftSpace*2, LFABtn_H);
        }else if (!cancelTitle && otherBtnTitle){
            _otherBtn.tag=0;
            _otherBtn.frame=CGRectMake(btnLeftSpace, btn_y, AlertView_W-btnLeftSpace*2, LFABtn_H);
        }else if (cancelTitle && otherBtnTitle){
            _cancelBtn.tag=0;
            _otherBtn.tag=1;
            CGFloat btnSpace = 20;//两个btn之间的间距
            CGFloat btn_w =(AlertView_W-btnLeftSpace*2-btnSpace)/2;
            _cancelBtn.frame=CGRectMake(btnLeftSpace, btn_y, btn_w, LFABtn_H);
            _otherBtn.frame=CGRectMake(_alertView.frame.size.width-btn_w-btnLeftSpace, btn_y, btn_w, LFABtn_H);
        }
...
-(void)setAnimationStyle:(LFAShowAnimationStyle)animationStyle{
